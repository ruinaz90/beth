...
recipe_urls = []
urls = [
    'https://www.budgetbytes.com/category/extra-bytes/budget-friendly-meal-prep/beef-meal-prep/',
...
for i in range(len(urls)):
    ...
    for recipe_title, recipe_url in recipe_dict.items():
        ...
        logging.debug(recipe_title)
        # Find each group of instructions
        instrgr = soup.findAll('div', {'class': 'wprm-recipe-instruction-group'})
        
        rec_id = soup.find("div", {"class": "wprm-recipe-container", "data-recipe-id": True})['data-recipe-id']
        logging.debug(rec_id)

        for instruct in instrgr:
            if (i > 1):
                # Check for extra sections 
                MultiSect = soup.findAll('h4', {'class': 'wprm-recipe-group-name wprm-recipe-instruction-group-name wprm-block-text-bold'})
                if len(MultiSect) > 0:
                    for m in MultiSect:
                        steps_list.append(m.text.strip())
                        logging.debug("----" + m.text)
            steps = instruct.findAll('li', {'class': 'wprm-recipe-instruction'})
            for step in steps:
                #logging.debug(step.text)
